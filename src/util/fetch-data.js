...
export async function fetchData(page = 1) {
  const response = await fetch(
    `https://bikeindex.org:443/api/v3/search?page=${page}&per_page=${bikesPerPage}&${searchParams}&stolenness=proximity`,
  );
...
export async function getBikesCount() {
  const response = await fetch(
    `https://bikeindex.org:443/api/v3/search/count?${searchParams}&stolenness=proximity`,
  );
